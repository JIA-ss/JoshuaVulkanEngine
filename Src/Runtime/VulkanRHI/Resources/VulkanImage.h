...
public:
    ...
    {
        vk::Extent3D extent;
        // vk::ImageSubresourceLayers  subresourceLayers = vk::ImageSubresourceLayers{vk::ImageAspectFlagBits::eColor,
        // 0, 0, 1};;
...
public:
    explicit VulkanImageResource(VulkanDevice* device, vk::MemoryPropertyFlags memProps, Config config);

    explicit VulkanImageResource(VulkanDevice* device, Native native);
    ~VulkanImageResource();
...
public:
    ...
    inline void SetNative(const Native& native) { m_native = native; }
...
public:
    explicit VulkanImageSampler(
        VulkanDevice* device, std::shared_ptr<Util::Texture::RawData> rawData, vk::MemoryPropertyFlags memProps,
        Config config, VulkanImageResource::Config resourceConfig);
    ~VulkanImageSampler();
