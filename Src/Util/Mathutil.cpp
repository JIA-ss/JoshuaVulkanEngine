...
Math::RTMatrix::RTMatrix(const glm::vec3& r, const glm::vec3& t, const glm::vec3& center)
    : m_rotation(r)
    , m_position(t)
    , m_center(center)
    , m_dirty(true)
...
void Math::RTMatrix::updateMatrix()
{
    ...
    m_matrix = glm::translate(glm::mat4(1.0f), m_center);
...
Math::VPMatrix::VPMatrix(
    ...
    , RTMatrix(rot, pos, center)
{
    glm::vec3 dir = m_center - m_position;
    glm::vec3 right = glm::cross(dir, m_upDirection);
    m_upDirection = glm::cross(right, dir);
}
...
void Math::SRTMatrix::updateMatrix()
{
    ...
    // std::cout << "model: " << glm::to_string(m_matrix) << std::endl;
    m_dirty = false;
}
